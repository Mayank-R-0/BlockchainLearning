...
contract ZombieAttack is ZombieHelper {
  ...
  function randMod(uint _modulus) internal returns(uint){
    randNonce = randNonce.add(1);                            // increamenting the ranNonce to pass new key everytime for new random number
  ...
  function attack (uint _zombieId, uint _targetId) external onlyOwnerOf(_zombieId){
    ...
    if(rand <= attackVictoryProbability) {                                          // checking if we win/loss
      myZombie.winCount = myZombie.winCount.add(1);                                 // incrementing my win count cause i win
      myZombie.level = myZombie.level.add(1);                                       // incrementing my level cause i win 
      enemyZombie.lossCount = enemyZombie.lossCount.add(1);                         // incrementing enemy's loss count cause they loss
      feedAndMultiply(_zombieId, enemyZombie.dna, "zombie");                        // create a new zombie based on the attack
    }
    else {
      myZombie.lossCount = myZombie.lossCount.add(1);                               // incrementing my loss count cause i loss
      enemyZombie.winCount = enemyZombie.winCount.add(1);                           // incrementing enemy win count cause they won
